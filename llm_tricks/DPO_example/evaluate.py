...
def evaluate_loss_dataloader(policy_model, reference_model, train_loader, val_loader, beta, eval_iter):
    ...
    with torch.no_grad():
        train_loss, train_chosen_rewards, train_rejected_rewards = compute_loss_dataloader(
            data_loader=train_loader, policy_model=policy_model, reference_model=reference_model, beta=beta, num_batches=eval_iter
        )
        val_loss, val_chosen_rewards, val_rejected_rewards = compute_loss_dataloader(
            data_loader=val_loader, policy_model=policy_model, reference_model=reference_model, beta=beta, num_batches=eval_iter
        )
    res = {
